...
def resize(x):
    x = scale(x, (full_dim, full_dim), 1)
    return tf.image.random_crop(x, (crop_dim, crop_dim, 3))
...
def gel_fulldata_v5():
  return base_v5().updated(
    ...
    resdir = './drive/My Drive/project/result/gel-128-v5/',
    batch_size = 16,
    step_size = 2000,
    train_iters = 7001,
...
def read_data(pr,sess):
  tf_files = [pj(pr.dsdir, 'train128_set2.tf'),pj(pr.dsdir, 'train128_set3.tf'),pj(pr.dsdir, 'train128_set4.tf')]
